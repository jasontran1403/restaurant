...
cartItems.forEach(item => {
  ...
  totalCell.textContent = formatPrice(item.quantity * item.price);
...
totalCell1.textContent = 'Tổng tiền: ';
  ...
  cartItems.forEach(function (cartItem) {
    ...
    row.innerHTML = `
      <td class="cart__product-item">
        <div class="cart__product-remove" onclick="removeCartItem(${cartItem.id})">
      ...
      <td class="cart__product-quantity">
        <div class="product-quantity">
          <div class="quantity__input-wrap">
            ...
            <input type="number" value="${cartItem.quan}" class="qty-input" onchange="updateQuantity(${cartItem.id}, this.value)" />
...
function updateQuantity(id, newQuantity) {
  ...
  if (item) {
    item.quan = parseInt(newQuantity);
  }
...
function updateQuantity(row, action) {
...
document.addEventListener("click", function(e) {
  if (e.target.classList.contains("increase-qty")) {
    ...
    updateQuantity(row, "increase");
  } else if (e.target.classList.contains("decrease-qty")) {
    e.preventDefault();
    var row = e.target.closest(".cart__product");
    updateQuantity(row, "decrease");
